...
const CHINESE_INFO_MESSAGE = (yearLength, year) =>
  `我用 Strava 记录自己户外运动 ${yearLength} 年了 ` + (year === 'Total' ? '，右侧地图展示的是所有的轨迹' : `，右侧地图展示的是 ${year} 年的轨迹`);

const ENGLISH_INFO_MESSAGE = (yearLength, year) =>
  `I've been recording my outdoor activities with Strava for ${yearLength} years` + (year === ', the map shows all routes' ? '' : `, the map shows routes in ${year}`);
